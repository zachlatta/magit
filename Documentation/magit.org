...
When working with Git it is often desirable if file-visiting buffers
are automatically reverted when the associated file changes on disk,
by updating its content to match the content of the modified file.
For example if you use an interactive rebase to correct a mistake you
made in an earlier commit, then it would be nice if the first thing
you had to do after rebase stopped at the commit were not having to
answer the question "File changed on disk; really edit the buffer?".

Magit used to provide its own implementation for reverting buffers
associated with a file located inside a Git repository.  But that
implementation was inefficient, so Magit now relies on the generic
implementation that is part of Emacs (implemented in ~autorevert.el~).

If you want all file-visiting buffers to be automatically reverted
when the accociated file changes, then enable ~global-auto-revert-mode~.
Otherwise enable ~magit-auto-revert-mode~ which turns on the local minor
mode ~auto-revert-mode~ in buffers associated with a file located inside
a Git repository.

If ~auto-revert-use-notify~ is non-nil (the default) then the reverting
starts shortly after the file changes on disk, otherwise that may be
delayed for up to ~auto-revert-interval~ seconds (five by default).  If
for some reason you cannot rely on file notifications, then you should
consider reverting at shorter intervals.
